...
{
   return chuong_trinh_phat_thanh_Get("SELECT chuong_trinh_phat_thanh.id, chuong_trinh_phat_thanh.active, chuong_trinh_phat_thanh.name, chuong_trinh_phat_thanh.name_url, chuong_trinh_phat_thanh.link_mp3, chuong_trinh_phat_thanh.noi_dung, chuong_trinh_phat_thanh.title, chuong_trinh_phat_thanh.keyword, chuong_trinh_phat_thanh.description FROM  chuong_trinh_phat_thanh ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function chuong_trinh_phat_thanh_insert($obj)
{
    return exe_query("insert into chuong_trinh_phat_thanh (active,name,name_url,link_mp3,noi_dung,title,keyword,description) values ('$obj->active','$obj->name','$obj->name_url','$obj->link_mp3','$obj->noi_dung','$obj->title','$obj->keyword','$obj->description')",'chuong_trinh_phat_thanh');
}
//
function chuong_trinh_phat_thanh_update($obj)
{
    return exe_query("update chuong_trinh_phat_thanh set active='$obj->active',name='$obj->name',name_url='$obj->name_url',link_mp3='$obj->link_mp3',noi_dung='$obj->noi_dung',title='$obj->title',keyword='$obj->keyword',description='$obj->description' where id=$obj->id",'chuong_trinh_phat_thanh');
}
//
