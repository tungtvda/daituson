...
{
   return tintuc_Get("SELECT tintuc.id, danh_muc_tin_tuc.name as danhmuc_id, tintuc.active, tintuc.name, tintuc.name_url, tintuc.img,tintuc.show_index, tintuc.tin_moi, tintuc.tin_noi_bat, tintuc.link_video, tintuc.file_mp4, tintuc.noi_dung, tintuc.title, tintuc.keyword, tintuc.description, tintuc.created FROM  tintuc, danh_muc_tin_tuc where danh_muc_tin_tuc.id=tintuc.danhmuc_id  ".(($where!='')?(' and '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
